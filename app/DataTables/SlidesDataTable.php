...
class SlidesDataTable extends DataTable
{
    ...
    public function dataTable(DataTables $dataTables, $query)
    {
        return datatables($query)
            ->addColumn('actions', 'admin.slides.buttons.actions')

            ->addColumn('image', '{!! view("admin.show_image",["image"=>$image])->render() !!}')

			->addColumn('page_name', function ($slide) {
				return Page::where('id', $slide->page_id)->firstOrFail()->page_name;	
			})
    ...
    {
        return Slide::query()->select("slides.*");
